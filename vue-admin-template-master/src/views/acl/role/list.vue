      ...
      <el-table-column label="角色名称">
        <template slot-scope="{row}">
          <template v-if="row.edit">
            <el-input v-model="row.roleName" class="edit-input" size="small" @keyup.enter.native="updateRole(row)"  ref="edit"/>
            <el-button
              class="cancel-btn"
              size="small"
    ...
    getRoles(page = 1) {
      ...
      this.$API.role.getPageList(page, limit, searchObj).then(
        result => {
          ...
          // map:返回一个新数组，不会修改原数组，且不会对空数组进行检测
