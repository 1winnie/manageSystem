  ...
  <div>
    <el-form label-width="80px" :model="spu">
      ...
      <el-form-item label="SPU照片墙">
        <!-- 上传图片：action：图片上传的地址，list-typr:文件列表的类型，on-preview:图片预览的时候会触发，on-remove：当删除图片的时候会触发
      ...
      <el-form-item label="销售属性">
        <el-select :placeholder="`还有${unSelectSaleAttr.length}未选择`" value="" v-model="attrIdAndAttrName">
...
export default {
  ...
  methods: {
    handleRemove(file, fileList) {
      // file参数:代表当前删除的那张图片
      // fileList参数：代表照片墙删除一张图片以后，剩余的其他图片
    ...
    handlePictureCardPreview(file) {
      this.dialogImageUrl = file.url;//将图片地址赋值给属性
      // 对话框显示
    ...
    addSaleAttr(){
      ...
      // 向spu对象的spuSaleAttrList属性里面添加新的属性对象
   ...
   async addOrUpdateSpu(){
      // 整理参数，需要整理照片墙的数据
      // 对于图片需要传递，spuName与imageUrl字段
      // 老图有name，和url属性，而新图有name和response.data,老图没有response
   ...
   async addSpuData(category3Id){
    ...
    cancel(){
      ...
      // 组件实例this._data,可以操作data中的响应数据
      // this.$options可以获取配置对象，配置对象的data函数执行，返回的响应式数据为空的（就是将已有属性的值全部变为空）
      Object.assign(this._data,this.$options.data());
    }
  },
  computed:{
    ...
    unSelectSaleAttr(){
      ...
      // 当前spu属于自己的销售属性在spu.spuSaleAttrList
      // 数组过滤，可以从已有的数组当中过滤出所需的数组，并返回一个新数组
